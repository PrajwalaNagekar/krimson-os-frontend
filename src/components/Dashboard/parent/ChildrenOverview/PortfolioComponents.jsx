...
import { Star, Calendar, Edit2, ExternalLink } from "lucide-react";

/**
 * Reusable Portfolio Card Component
 * Displays a single portfolio item with thumbnail, title, date, tags
  ...
  return (
    <div className="group bg-white rounded-2xl overflow-hidden shadow-sm border border-slate-100 hover:shadow-lg hover:-translate-y-1 transition-all duration-300">
      ...
      <div className="relative h-40 overflow-hidden bg-slate-100">
        ...
        {/* Showcase Toggle - Only in edit mode */}
        {!readOnly && onToggleShowcase && (
          <button
      ...
      <div className="p-5">
        <div className="flex justify-between items-start mb-2">
          <h3
            className="font-bold text-slate-800 line-clamp-1 group-hover:text-blue-600 transition-colors"
            title={item.title}
          >
            {item.title}
          </h3>
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {item.tags.map((tag, i) => (
            <span
              key={i}
              className={`text-[10px] font-bold px-2 py-1 rounded-md uppercase tracking-wide border ${
                i % 2 === 0
...
/**
 * Reusable Portfolio Section Component
 * Groups portfolio items by category with icon and color customization
 * Supports both editable and read-only modes
 */
