...
const DiscussionHub = ({ initialData }) => {
  ...
  const availableTeachers = initialData?.availableTeachers || [];
  ...
  const filteredSpaces = discussionSpaces.filter(
    (space) =>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar">
          {filteredSpaces.map((space, index) => (
            <div
              ...
              className={`p-4 border-b border-slate-200 cursor-pointer transition-all duration-300 hover:bg-white hover:shadow-sm animate-in fade-in slide-in-from-left-4 ${
            ...
            >
              <div className="flex items-start gap-3">
                <div
                  className={`w-12 h-12 rounded-full bg-gradient-to-br ${space.color} text-white flex items-center justify-center font-bold text-lg flex-shrink-0 shadow-md hover:shadow-lg transition-shadow`}
                >
                  {space.avatar}
                </div>
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-medium text-indigo-600 px-2 py-0.5 bg-indigo-50 rounded-lg">
                      {space.topic}
                    </span>
                    {space.unread > 0 && (
        ...
        <div className="flex-1 flex flex-col bg-gradient-to-b from-slate-50/50 to-white">
          ...
          <div className="p-4 border-b border-slate-200 bg-white/80 backdrop-blur-xl sticky top-0 z-10 shadow-sm">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div
                  className={`w-12 h-12 rounded-full bg-gradient-to-br ${selectedSpace.color} text-white flex items-center justify-center font-bold text-lg shadow-md`}
                >
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 text-xs text-slate-500 mt-0.5">
                    <Users size={14} />
                    <span>
                      {selectedSpace.participants.length} participants
                    </span>
                    <span>•</span>
                    <span className="px-2 py-0.5 rounded-lg bg-indigo-50 text-indigo-600 font-medium">
                      {selectedSpace.topic}
                    </span>
                  </div>
                </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar">
            {(messages[selectedSpace.id] || []).map((message, index) => (
              ...
              >
                <div
                  className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm flex-shrink-0 shadow-md transition-transform hover:scale-110 ${
                    message.isOwn
                ...
                <div
                  className={`flex-1 max-w-lg ${message.isOwn ? "items-end" : ""}`}
                >
                  {!message.isOwn && (
                  ...
                  >
                    ...
                    {message.attachment && (
                      <div
                        className={`mt-3 p-3 rounded-xl flex items-center gap-2 transition-all hover:scale-105 cursor-pointer ${
                          message.isOwn
                            ? "bg-indigo-500/50 backdrop-blur-sm"
                  ...
                  <div
                    className={`flex items-center gap-1 mt-1 ${message.isOwn ? "justify-end" : ""}`}
                  >
        ...
        <div className="flex-1 flex items-center justify-center bg-gradient-to-br from-slate-50 to-indigo-50/20">
          <div className="text-center animate-in fade-in zoom-in duration-500">
            ...
            <p className="text-slate-500">
              Choose a space from the left to start collaborating
            </p>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl p-8 max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl animate-in zoom-in slide-in-from-bottom-4 duration-300">
            ...
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                  {topics.map((topic, index) => (
                    <button
                      ...
                      className={`p-3 rounded-xl text-xs font-bold transition-all animate-in fade-in zoom-in hover:scale-105 active:scale-95 ${
                        newSpace.topic === topic
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-h-64 overflow-y-auto p-1 custom-scrollbar">
                  {availableTeachers.map((teacher, index) => (
                    <div
                      ...
                      className={`p-3 rounded-xl border-2 cursor-pointer transition-all animate-in fade-in slide-in-from-bottom-2 hover:scale-105 active:scale-95 ${
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div
                          className={`w-10 h-10 rounded-full bg-gradient-to-br ${teacher.color} text-white flex items-center justify-center font-bold text-sm shadow-md`}
                        >
                          {teacher.avatar}
                        </div>
              ...
              {/* Actions */}
              <div
                className="flex gap-3 pt-4 animate-in slide-in-from-bottom-4"
