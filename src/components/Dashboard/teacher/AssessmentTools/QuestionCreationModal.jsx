...
const QuestionCreationModal = ({ isOpen, onClose, onSave }) => {
  ...
  const bloomLevels = assessmentTools.bloomLevels;
  // Map difficultyLevels from objects to strings for the select option values if needed
  // The data has objects {id, label, color}. The original code used strings ['Basic', 'Standard', 'Advanced']
  // Let's map to labels or ids. IDs seem to match labels in data.
  const difficultyLevels = assessmentTools.difficultyLevels.map((l) => l.id);
  ...
  const handleAiDraft = () => {
    ...
    setTimeout(() => {
      setAiSuggestion(
        "Suggested refinement: 'Formulate a step-by-step procedure to isolate the variable x in a linear equation with fractional coefficients.'",
      );
    }, 1000);
  };
        ...
        <div className="p-8 border-b border-slate-100 flex items-center justify-between bg-gradient-to-r from-slate-50 to-white">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span className="px-2 py-0.5 bg-cyan-100 text-cyan-600 rounded-lg text-[10px] font-black uppercase tracking-widest">
                Screen 200
              </span>
            ...
            <p className="text-slate-500 text-sm font-medium">
              Add to your personal and school library
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-8 space-y-8 flex-1">
          ...
          <div className="bg-gradient-to-r from-cyan-500 to-blue-600 rounded-3xl p-6 text-white shadow-lg relative overflow-hidden group">
            ...
            <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div>
                ...
                <p className="text-sm opacity-80 font-medium">
                  Draft questions, refine wording, or align with Bloom's
            ...
            {aiSuggestion && (
              <div className="mt-4 pt-4 border-t border-white/20 animate-in slide-in-from-top-2">
                <p className="text-sm italic font-medium bg-white/10 p-4 rounded-2xl border border-white/10">
                  {aiSuggestion}
                </p>
              </div>
            )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  <label className="text-xs font-black text-slate-400 uppercase tracking-widest pl-1 text-cyan-500">
                    Outcome
                  </label>
                  <input
                    type="text"
                ...
                <div className="space-y-2">
                  <label className="text-xs font-black text-slate-400 uppercase tracking-widest pl-1 text-blue-500">
                    Bloom's level
                  </label>
                      ...
                      <div key={idx} className="flex items-center gap-3 group">
                        <button
                          className={`w-8 h-8 rounded-full border-2 flex items-center justify-center text-[10px] font-black transition-all ${
                            formData.correctAnswer === idx.toString()
