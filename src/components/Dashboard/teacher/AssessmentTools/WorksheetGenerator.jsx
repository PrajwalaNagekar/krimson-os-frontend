...
const WorksheetGenerator = () => {
  ...
  const [savedWorksheets] = useState(assessmentTools.savedWorksheets);

  // Filter questions that match the worksheet generator context (ids 4 and 5 in the data)
  // In a real app, this would be a filter or API call
  const [questions, setQuestions] = useState(
    assessmentTools.questions.filter((q) => q.id === 4 || q.id === 5),
  );
  ...
  const difficultyLevels = assessmentTools.difficultyLevels;
  const types = assessmentTools.questionTypes;
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {savedWorksheets.map((ws) => (
              <div
                key={ws.id}
                className="bg-white p-6 rounded-[32px] shadow-sm border-2 border-slate-100 hover:border-cyan-200 transition-all group relative overflow-hidden flex flex-col"
              >
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span
                    className={`px-3 py-1 rounded-full text-[10px] font-black uppercase border tracking-widest ${difficultyLevels.find((l) => l.id === ws.difficulty)?.color}`}
                  >
                    {ws.difficulty}
                  </span>
                </div>

                <h4 className="text-lg font-black text-slate-800 mb-2 leading-tight group-hover:text-cyan-600 transition-colors">
                  {ws.title}
                </h4>
            ...
            >
              ...
              <p className="font-black text-slate-400 uppercase tracking-widest text-xs group-hover:text-cyan-500">
                Create New Sheet
              </p>
            </div>
          </div>
            ...
            <div className="bg-white p-6 md:p-8 rounded-3xl shadow-sm border-2 border-slate-100 flex flex-col h-full">
              ...
              <div className="space-y-6 flex-1">
                  ...
                  <div className="flex flex-wrap gap-3">
                    {types.map((type) => (
                      <button
                        key={type.id}
                        onClick={() => {
                          const newTypes = formData.questionTypes.includes(
                            type.id,
                          )
                        ...
                        className={`px-4 py-3 rounded-2xl border-2 font-bold text-sm transition-all flex items-center gap-2 ${
                ...
                <div className="space-y-2">
                  ...
                  <div className="space-y-2">
                    {[
                      "M9.1.A: Solve multi-step equations",
                      "M9.1.B: Map real world to algebra",
                    ].map((outcome) => (
                      <div
                        key={outcome}
                        className="flex items-center gap-3 p-4 bg-slate-50 rounded-2xl border border-slate-100 group cursor-pointer hover:bg-cyan-50 hover:border-cyan-200 transition-all"
                      >
                        <input
                          type="checkbox"
                          className="w-5 h-5 rounded-lg border-2 border-slate-300 text-cyan-500 focus:ring-cyan-500 transition-all cursor-pointer"
                        />
                        <span className="text-sm font-semibold text-slate-600 group-hover:text-cyan-700 transition-colors uppercase tracking-tight">
                          {outcome}
                        </span>
                      </div>
                    ))}
            ...
            <div className="bg-slate-900 rounded-3xl p-6 md:p-8 text-white shadow-xl relative overflow-hidden group">
              ...
              <div className="relative z-10 space-y-4">
                ...
                <h4 className="font-bold text-lg leading-snug">
                  Choose "Advanced" for remediation work.
                </h4>
                <p className="text-sm text-slate-400 font-medium">
                  AI will suggest multi-step problems that challenge
                  high-performing students. For struggling students, stick to
                  "Basic" reinforcement.
                </p>
              </div>
            </div>
            ...
            <div className="bg-white p-6 rounded-3xl shadow-sm border-2 border-slate-100 space-y-6">
              ...
              <div className="space-y-3">
                {[
                  ...
                  <div
                    key={idx}
                    className="p-4 bg-slate-50 rounded-2xl border border-slate-100 hover:bg-slate-100 transition-all cursor-pointer group"
                  >
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mt-0.5">
                  <span
                    className={`px-2 py-0.5 rounded text-[10px] font-black uppercase ring-1 ring-inset ${
                      difficultyLevels.find((l) => l.id === formData.difficulty)
                        ?.color
                    }`}
                  >
                    {formData.difficulty}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
            ...
            <div className="lg:col-span-3 space-y-4">
              {questions.map((q, idx) => (
                <div
                  key={q.id}
                  className="bg-white p-6 rounded-3xl shadow-sm border-2 border-slate-100 group hover:border-cyan-200 transition-all"
                >
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-2">
                        {q.options.map((opt, oIdx) => (
                          ...
                          >
                            ...
                            <span
                              className={`text-sm font-bold ${opt === q.answer ? "text-cyan-700" : "text-slate-600"}`}
                            >
                              {opt}
                            </span>
                          </div>
                ...
                <div className="pt-4 border-t border-slate-100">
                  <div className="bg-cyan-50 p-4 rounded-2xl border border-cyan-100">
                    ...
                    <p className="text-[10px] text-cyan-700 font-medium leading-relaxed">
                      AI2 found 3 questions that match your recent quiz. Should
                      I replace them to maintain variety?
                    </p>
                    <button className="mt-3 text-[10px] font-black text-cyan-600 uppercase hover:underline">
                      Apply Variations
                    </button>
                  </div>
                </div>
