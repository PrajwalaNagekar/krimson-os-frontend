...
const QuestionBank = () => {
  const { assessmentTools } = TEACHER_DATA;
  const [showCreateModal, setShowCreateModal] = useState(false);
  const [viewMode, setViewMode] = useState("list"); // 'grid' or 'list'

  // Use questions 1, 2, 3 from the data for the question bank view
  const [questions] = useState(
    assessmentTools.questions.filter((q) => q.id <= 3),
  );

  const [searchQuery, setSearchQuery] = useState("");

  const getDifficultyColor = (diff) => {
    const level = assessmentTools.difficultyLevels.find((l) => l.id === diff);
    // Map the data color format to the class names expected here, or use the color directly if compatible
    // The data has full classes string "bg-emerald-50 text-emerald-600 border-emerald-200"
    // The component expects "text-emerald-500 bg-emerald-50 border-emerald-100" structure in switch
    // Let's adapt the function to use the data if possible, or keep the helper if the data format differs significantly.
    // Upon review, the data format is slightly different. Let's keep a local helper that maps or uses the data.
    // For simplicity and to match the data structure, let's use the data's color property directly.
        ...
        <div className="flex-1 flex flex-col md:flex-row gap-4">
          ...
          <div className="flex-1 relative group">
            ...
            <div className="absolute right-4 top-1/2 -translate-y-1/2 flex items-center gap-1">
              ...
              <span className="text-[10px] font-black text-cyan-400 uppercase tracking-widest hidden md:inline">
                AI6 Powered
              </span>
            </div>
          </div>
        ...
        {viewMode === "list" ? (
                ...
                <tbody className="divide-y divide-slate-100">
                  {questions.map((q, idx) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-6 max-w-lg">
                        <div className="flex flex-col gap-1.5">
                          <div className="flex items-center gap-2">
                            <span className="px-2 py-0.5 bg-slate-100 text-slate-500 rounded-lg text-[10px] font-black uppercase tracking-widest border border-slate-200">
                              {q.type}
                            </span>
                            <span className="px-2 py-0.5 bg-purple-100 text-purple-600 rounded-lg text-[10px] font-black uppercase tracking-widest border border-purple-200">
                              {q.bloom}
                            </span>
                          </div>
                      ...
                      <td className="px-6 py-6 text-center">
                        <span
                          className={`px-3 py-1.5 rounded-full text-[10px] font-black uppercase border tracking-widest ${getDifficultyColor(q.difficulty)}`}
                        >
                          {q.difficulty}
                        </span>
                      </td>
                      ...
                      <td className="px-6 py-6 text-center">
                        <div className="flex items-center justify-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button className="p-2.5 bg-slate-100 text-slate-500 rounded-xl hover:text-cyan-600 hover:bg-white transition-all shadow-sm">
                            <Eye size={16} />
                          </button>
                          <button className="p-2.5 bg-slate-100 text-slate-500 rounded-xl hover:text-blue-600 hover:bg-white transition-all shadow-sm">
                            <Edit size={16} />
                          </button>
                          <button className="p-2.5 bg-slate-100 text-slate-500 rounded-xl hover:text-red-600 hover:bg-white transition-all shadow-sm">
                            <Trash2 size={16} />
                          </button>
                        </div>
                      </td>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {questions.map((q, idx) => (
              <div
                key={idx}
                className="bg-white p-6 rounded-[32px] shadow-sm border-2 border-slate-100 hover:border-cyan-200 transition-all group relative overflow-hidden"
              >
                <div className="absolute top-0 right-0 p-4 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button className="p-2 bg-slate-100 text-slate-400 rounded-xl hover:bg-white hover:text-cyan-600 shadow-sm">
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-2">
                    <span className="px-2 py-0.5 bg-slate-50 text-slate-500 rounded-lg text-[10px] font-black uppercase border border-slate-100">
                      {q.type}
                    </span>
                    <span
                      className={`px-2 py-0.5 rounded-lg text-[10px] font-black uppercase border ${getDifficultyColor(q.difficulty)}`}
                    >
                      {q.difficulty}
                    </span>
                  </div>

                  <p className="text-base font-bold text-slate-800 leading-snug line-clamp-3 min-h-[4.5rem] group-hover:text-cyan-600 transition-colors">
                    {q.text}
                  </p>
      ...
      <div className="bg-gradient-to-br from-slate-900 to-slate-800 rounded-3xl p-6 md:p-8 text-white shadow-xl relative overflow-hidden group">
        ...
        <div className="flex flex-col md:flex-row items-center gap-6 relative z-10">
          ...
          <div className="flex-1 text-center md:text-left">
            <h4 className="font-black uppercase tracking-widest text-cyan-400 text-xs mb-2">
              Duplicate Detection Engine
            </h4>
            <h5 className="text-xl font-bold mb-3">
              AI6 detected 4 potential duplicate questions.
            </h5>
            <p className="text-sm text-slate-400 font-medium max-w-2xl">
              We've found similar wording and logic in the school library. Would
              you like to merge them or archive your local copies to maintain
              question integrity?
            </p>
          </div>
