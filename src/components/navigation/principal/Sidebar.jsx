...
const PrincipalSidebar = ({ isOpen, onClose }) => {
  ...
  const menuItems = [
    ...
    },
    {
      ...
      path: "/dashboard/principal/welfare",
      icon: Heart,
    },
    {
  ...
  const isActive = (path) => {
    if (
      ...
      return true;
    if (path !== "/dashboard/principal" && location.pathname.startsWith(path))
      return true;
    return false;
  };
  ...
  return (
    <>
      ...
      {/* Sidebar Container */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 sidebar-width-mobile p-3 md:p-4 transition-all duration-300 ${
        ...
        <div className="h-full w-full rounded-3xl bg-gradient-to-b from-cyan-400 via-blue-400 to-pink-400 p-4 flex flex-col text-white shadow-2xl relative overflow-hidden">
          ...
          <div className="relative z-10 flex-1 flex flex-col overflow-hidden">
            ...
            <div className="flex items-center gap-2 md:gap-3 mb-6 md:mb-8 shrink-0 pl-1 md:pl-2">
              ...
              <div>
                <h1 className="font-bold text-base md:text-lg tracking-wide text-white">
                  Krimson OS
                </h1>
            ...
            <nav className="flex-1 overflow-y-auto space-y-1 custom-scrollbar py-2 min-h-0 pr-1">
              {menuItems.map((item) => {
                ...
                return (
                  ...
                  >
                    <Icon
                      className={`h-4 w-4 md:h-5 md:w-5 flex-shrink-0 ${active ? "text-blue-500" : "text-white"}`}
                    />
