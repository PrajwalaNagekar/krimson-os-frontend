...
import { Calendar, CheckCircle, XCircle, Clock, User, BarChart3, Info } from 'lucide-react';
...
const CCAAttendanceSection = ({ ccaAttendance }) => {
  const [selectedActivity, setSelectedActivity] = useState(ccaAttendance?.activities?.[0] || null);
    ...
    <div className="bg-white rounded-3xl shadow-md p-6 md:p-8">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-2xl font-bold text-slate-800">CCA Attendance</h2>
            <p className="text-sm text-slate-500">Track your activity participation</p>
          </div>
        </div>
        {/* Overall Attendance Badge */}
        <div className="text-center bg-gradient-to-br from-blue-50 to-cyan-50 rounded-2xl px-6 py-3 border border-blue-100">
          <p className="text-xs text-blue-600 font-semibold mb-1">Overall Attendance</p>
          <p className="text-3xl font-bold text-blue-700">{overview.attendancePercentage}%</p>
          <p className="text-xs text-slate-500 mt-1">{overview.attendedSessions}/{overview.totalSessions} sessions</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-3">
          ...
          {activities.map((activity) => {
            ...
            const attendanceColor = activity.attendancePercentage >= 90
              ? 'green'
              ...
              >
                <div className="flex items-start justify-between mb-2">
                  <div className="flex-1">
                    <h4 className="font-bold text-slate-800 text-sm mb-1">{activity.name}</h4>
                    <p className="text-xs text-slate-500">{activity.category}</p>
                  </div>
                  <div className={`text-${attendanceColor}-600 bg-${attendanceColor}-50 px-2 py-1 rounded-lg`}>
                    <span className="text-xs font-bold">{activity.attendancePercentage}%</span>
                  </div>
                </div>
        ...
        <div className="lg:col-span-2">
          {selectedActivity ? (
            <div className="bg-gradient-to-br from-slate-50 to-blue-50 rounded-2xl p-6 border border-slate-200">
              {/* Activity Header */}
              <div className="mb-6">
                <h3 className="text-xl font-bold text-slate-800 mb-2">{selectedActivity.name}</h3>
              ...
              <div className="grid grid-cols-3 gap-3 mb-6">
                <div className="bg-white rounded-xl p-4 text-center border border-green-100">
                  ...
                  <p className="text-2xl font-bold text-green-600">{selectedActivity.attendedSessions}</p>
                ...
                <div className="bg-white rounded-xl p-4 text-center border border-red-100">
                  ...
                  <p className="text-2xl font-bold text-red-600">{selectedActivity.missedSessions}</p>
                ...
                <div className="bg-white rounded-xl p-4 text-center border border-blue-100">
                  ...
                  <p className="text-2xl font-bold text-blue-600">{selectedActivity.attendancePercentage}%</p>
                          ...
                          <div className="flex-1">
                            ...
                            {session.reason && (
                              <div className="mt-2 flex items-center gap-2 bg-orange-50 px-3 py-1.5 rounded-lg inline-block">
                                ...
                                <span className="text-xs text-orange-700">Reason: {session.reason}</span>
                              </div>
                            )}
                          </div>
                        </div>
            ...
            <div className="bg-slate-50 rounded-2xl p-12 text-center border border-slate-200">
              ...
              <p className="text-slate-400 font-medium">Select an activity to view attendance details</p>
            </div>
          )}
        </div>
      </div>
