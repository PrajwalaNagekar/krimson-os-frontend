...
import { Mail, Phone, MapPin, Award, BookOpen, Edit2 } from 'lucide-react';

const ProfileHeader = ({ user, profile, onEdit, onEditClub, onDeleteClub }) => {
    ...
    <div className="relative bg-white rounded-3xl overflow-hidden shadow-sm border border-slate-100 mb-8 group hover:shadow-md transition-all duration-300">
      ...
      <div className="px-8 pb-8">
        ...
        <div className="bg-slate-50 rounded-2xl p-6 border border-slate-100/60 mb-6">
            <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-3">About My Learning</h3>
            ...
            {user.skills && user.skills.length > 0 && (
                <div className="mt-4 flex flex-wrap gap-2">
                    {user.skills.map((skill, index) => (
                        <span key={index} className="px-3 py-1 bg-white border border-slate-200 rounded-lg text-xs font-bold text-slate-600 shadow-sm">
                            #{skill}
                        </span>
                    ))}
                </div>
